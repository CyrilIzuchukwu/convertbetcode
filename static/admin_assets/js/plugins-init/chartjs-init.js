    ...
    var dlabSparkLine = function () {
        ...
        var lineChart1 = function () {
            ...
            if (jQuery('#lineChart_1').length > 0) {
                ...
                const values = JSON.parse(chartDataElement.getAttribute('data-values'));
                //basic line chart
                        ...
                        scales: {
                            y: {
                                ...
                                ticks: {
                                    beginAtZero: false,
                                    // max: 100,
                                    max: Math.ceil(Math.max(...values) / 1000),
                                    min: 0,
                                    stepSize: 20,
                                    padding: 10,
                                    callback: function (value) {
                                        // Only add 'k' if the value is not 0
        ...
        var doughnutChart = function () {
            if (jQuery('#doughnut_chart').length > 0) {
                ...
                const dataValues = JSON.parse(doughnutChartElement.getAttribute('doughnut-data-values'));
                // Doughnut chart
                        ...
                        plugins: {
                            datalabels: {
                                ...
                                font: {
                                    size: 12,
                                    weight: 700,
                                    family: 'Poppins, sans-serif' // Specify your font family here
                                },
                                anchor: 'center',
                                align: 'center',
                                formatter: (value, context) => {
                                    const total = context.chart._metasets[0].total;
                                    const percentage = Math.round((value / total) * 100) + '%'; // Round to nearest whole number
                                    return percentage;
                                }
                            }
                        }
